...
import { tailLayout, formItemLayoutTan } from '@/utils/helper'
...
class Ipv4 extends React.Component {
  ...
  getData = () => {
    post('/admin/cfg.php?controller=network&action=showRouteIPv6').then(
      (res) => {
  ...
  handleSaveBtn = (values) => {
    ...
    if (this.state.id == '') {
      post(
        '/admin/cfg.php?controller=network&action=addRouteIPv6',
        values
      ).then((res) => {
        if (res.success) {
          ...
          <Form {...formItemLayoutTan} {...formProps}>
            ...
            >
              <Input
                className="Ip_70"
                suffix={
                  <Tooltip title="例如 3001:33b8::0000/64">
            ...
            <Form.Item {...tailLayout}>
              <Button type="primary" className={styles.Po_Le} htmlType="submit">
                保存
              </Button>
