let serverSupportsSSL = false
let disableSSLMenu = false
let suppressAllAudio = true
let linkifyURLs = 1
let userCSSList = []
let serverMessagesOrigin = false
import './shellbox.css'
...
VT100.prototype.reset = function (clearHistory) {
  ...
  this.printing = false
  if (
    typeof this.printWin != 'undefined' &&
    this.printWin &&
    !this.printWin.closed
  ) {
    this.printWin.close()
  }
  this.printWin = null
...
VT100.prototype.getUserSettings = function () {
  ...
  if (typeof userCSSList != 'undefined') {
    for (var i = 0; i < userCSSList.length; ++i) {
      ...
      for (var j = 0; j < label.length; ++j) {
        this.signature =
          Math.floor(16807 * this.signature + label.charCodeAt(j)) %
          ((1 << 31) - 1)
      }
      if (userCSSList[i][1]) {
        this.signature =
          Math.floor(16807 * this.signature + 1) % ((1 << 31) - 1)
      }
    }
  }
  ...
  if (settings >= 0) {
    settings = document.cookie
      .substr(settings + key.length)
      .replace(/([0-1]*).*/, '$1')
    if (
      settings.length ==
      6 + (typeof userCSSList == 'undefined' ? 0 : userCSSList.length)
    ) {
    ...
    for (var i = 0; i <= userCSSList.length; ++i) {
      ...
      if (newGroup || i == userCSSList.length) {
        ...
        for (var j = beginOfGroup; j < i; ++j) {
          this.usercssActions[this.usercssActions.length] = (function (
            vt100,
            current,
            begin,
            count
            ...
            return function () {
                ...
                if (entry.tagName == 'LI') {
                  if (++i >= begin) {
                    ...
                    if (i == current) {
                      ...
                      if (!sheet.disabled) {
                        label.innerHTML =
                          '<img src="enabled.gif" />' + label.innerHTML
                      }
                    } else {
                      sheet.disabled = true
                    }
              ...
              for (i = 0; i < this.console.length; ++i) {
                for (
                  var line = this.console[i].firstChild;
                  line;
      ...
      group +=
        '<li>' + (enabled ? '<img src="enabled.gif" />' : '') + label + '</li>'
    }
...
VT100.prototype.resetLastSelectedKey = function (e) {
  ...
  if (
    position[0] < box.offsetLeft + key.offsetWidth ||
...
VT100.prototype.clickedKeyboard = function (
  e,
  elem,
  ch,
  key,
      ...
      if (elem.id) {
        ...
        if (child) {
          if (child.nodeName == '#text') {
            ...
            // key.
            var text = this.getTextContent(child) || this.getTextContent(elem)
...
VT100.prototype.initializeElements = function (container) {
  ...
  if (
    !this.getChildById(this.container, 'reconnect') ||
    ...
    try {
      if (
        typeof navigator.mimeTypes['audio/x-wav'].enabledPlugin.name !=
        'undefined'
      ) {
        embed =
          typeof suppressAllAudio != 'undefined' && suppressAllAudio
            ? ''
  ...
  var marginTop = parseInt(
    this.getCurrentComputedStyle(document.body, 'marginTop')
  )
  var marginLeft = parseInt(
    this.getCurrentComputedStyle(document.body, 'marginLeft')
  )
  var marginRight = parseInt(
    this.getCurrentComputedStyle(document.body, 'marginRight')
  )
  ...
  this.isEmbedded =
    marginTop != y || marginLeft != x || (widthDiff != 0 && widthDiff != -1)
  if (!this.isEmbedded) {
  ...
  this.addListener(
    this.scrollable,
    'mousedown',
    mouseEvent(this, 0 /* MOUSE_DOWN */)
  )
  this.addListener(
    this.scrollable,
    'mouseup',
    mouseEvent(this, 1 /* MOUSE_UP */)
  )
  this.addListener(
    this.scrollable,
    'click',
    mouseEvent(this, 2 /* MOUSE_CLICK */)
  )
...
VT100.prototype.resizer = function () {
  ...
  this.reconnectBtn.style.left =
    ((this.terminalWidth * this.cursorWidth) / this.scale -
      this.reconnectBtn.clientWidth) /
      2 +
    'px'
  this.reconnectBtn.style.top =
    (this.terminalHeight * this.cursorHeight - this.reconnectBtn.clientHeight) /
      2 +
    'px'
...
VT100.prototype.showCurrentSize = function () {
  ...
  this.curSizeBox.innerHTML =
    '' + this.terminalWidth + 'x' + this.terminalHeight
  this.curSizeBox.style.left =
    ((this.terminalWidth * this.cursorWidth) / this.scale -
      this.curSizeBox.clientWidth) /
      2 +
    'px'
  this.curSizeBox.style.top =
    (this.terminalHeight * this.cursorHeight - this.curSizeBox.clientHeight) /
      2 +
    'px'
  ...
  this.curSizeTimeout = setTimeout(
    (function (vt100) {
      return function () {
...
VT100.prototype.mousePosition = function (event) {
  ...
  return [event.clientX - offsetX, event.clientY - offsetY]
}
...
VT100.prototype.mouseEvent = function (event, type) {
  ...
  if (
    (type == 1 /* MOUSE_UP */ || type == 2) /* MOUSE_CLICK */ &&
    !selection.length
  ) {
    this.input.focus()
  }
  ...
  var y =
    Math.floor((position[1] + this.scrollable.scrollTop) / this.cursorHeight) -
    this.numScrollbackLines
  var inside = true
...
VT100.prototype.htmlEscape = function (s) {
  return this.replaceChar(
    this.replaceChar(
      this.replaceChar(this.replaceChar(s, '&', '&amp;'), '<', '&lt;'),
      '"',
      '&quot;'
    ),
    ' ',
  ...
  if (typeof elem.textContent == 'undefined') {
    if (elem.innerText != s) {
      try {
        ...
        elem.innerHTML = ''
        elem.appendChild(
          document.createTextNode(this.replaceChar(s, ' ', '\u00A0'))
        )
      }
    }
  } else {
    ...
    for (;;) {
      ...
      if (
        ...
        if (
          ...
          fullUrl = (url.indexOf('ftp.') == 0 ? 'ftp://' : 'http://') + url
        }
      }
...
VT100.prototype.updateWidth = function () {
  ...
  if (this.cursorWidth <= 0) {
    if (this.cursor.clientWidth <= 0) {
      ...
      // update the size.
      this.cursorWidth = this.cursor.clientWidth
      this.terminalWidth = Math.floor(
        (this.console[this.currentScreen].offsetWidth / this.cursorWidth) *
          this.scale
      )
    }
  } else {
...
VT100.prototype.updateHeight = function () {
  ...
  if (this.isEmbedded) {
    ...
    this.terminalHeight = Math.floor(
      (this.container.clientHeight - 1) / this.cursorHeight
    )
  } else {
...
VT100.prototype.updateNumScrollbackLines = function () {
  var scrollback =
    Math.floor(
      this.console[this.currentScreen].offsetHeight / this.cursorHeight
    ) - this.terminalHeight
...
VT100.prototype.truncateLines = function (width) {
  ...
  for (
    var line = this.console[this.currentScreen].firstChild;
    line;
    line = line.nextSibling
  ) {
    if (line.tagName == 'DIV') {
...
VT100.prototype.putString = function (x, y, text, color, style) {
  ...
  if (
    !text.length &&
    (yIdx >= console.childNodes.length ||
    ...
    if (text.length) {
      ...
      if (
        ...
        this.setTextContent(
          span,
          this.getTextContent(span) + this.getTextContent(sibling)
        )
        line.removeChild(sibling)
      }
    }
  ...
  if (!this.cursor.style.visibility) {
    ...
    if (span) {
      ...
      pixelY = span.offsetTop + span.offsetParent.offsetTop -100 
      s = this.getTextContent(span)
  ...
  if (pixelX >= 0) {
    this.cursor.style.left = (pixelX + (this.isIE ? 1 : 0)) / this.scale + 'px'
  } else {
    this.setTextContent(this.space, this.spaces(this.cursorX))
    this.cursor.style.left =
      (this.space.offsetWidth + console.offsetLeft) / this.scale + 'px'
  }
  this.cursorY = yIdx - this.numScrollbackLines
  if (pixelY >= 0) {
    this.cursor.style.top = pixelY + 'px'
  } else {
    this.cursor.style.top = yIdx * this.cursorHeight + console.offsetTop + 'px'
  }
  ...
  if (text.length) {
    ...
    if (
      ...
      this.setTextContent(
        span,
        this.getTextContent(sibling) + this.getTextContent(span)
      )
      line.removeChild(sibling)
    }
...
VT100.prototype.refreshInvertedState = function () {
  if (this.isInverted) {
    ...
    this.scrollable.className = this.scrollable.className.replace(
      / *inverted/,
      ''
    )
  }
...
VT100.prototype.enableAlternateScreen = function (state) {
  ...
  if (transform) {
    ...
    var style = this.console[this.currentScreen].style[transform]
...
VT100.prototype.scrollFore = function () {
  ...
  this.scrollable.scrollTop =
    i > this.numScrollbackLines * this.cursorHeight + 1
      ? this.numScrollbackLines * this.cursorHeight + 1
      : i
}
...
VT100.prototype.clearRegion = function (x, y, w, h, color, style) {
  ...
  if (
    ...
    w == this.terminalWidth &&
    h == this.terminalHeight &&
    (color == undefined || color == 'ansiDef bgAnsiDef') &&
    !style
  ) {
    var console = this.console[this.currentScreen]
...
VT100.prototype.scrollRegion = function (x, y, w, h, incX, incY, color, style) {
  ...
  if (!dontScroll) {
    ...
    if (!incX && !x && w == this.terminalWidth) {
      ...
      if (incY < 0) {
        ...
        if (
          ...
          // printed.
          for (
            var i = this.numScrollbackLines, j = -incY;
            i-- > 0 && j-- > 0;
          ...
          for (
            var i = -incY;
            i-- > 0 &&
            console.childNodes.length > this.numScrollbackLines + y + incY;

          ) {
            console.removeChild(
              console.childNodes[this.numScrollbackLines + y + incY]
            )
          }
        ...
        for (
          ...
          console.removeChild(
            console.childNodes[this.numScrollbackLines + y + h]
          )
        }
        for (var i = incY; i--; ) {
      ...
      if (incY <= 0) {
        ...
        for (
          var i = y + this.numScrollbackLines;
        ...
        // Scrolling down
        for (
          var i = y + this.numScrollbackLines + h;
    ...
    this.scrollable.scrollTop =
      (this.numScrollbackLines - scrollPos) * this.cursorHeight + 1
...
VT100.prototype.showSoftKeyboard = function () {
  ...
  if (transform) {
    ...
    if (kbd.offsetWidth > 0.9 * this.container.offsetWidth) {
      scale = kbd.offsetWidth / this.container.offsetWidth / 0.9
    }
    if (kbd.offsetHeight > 0.9 * this.container.offsetHeight) {
      scale = Math.max(kbd.offsetHeight / this.container.offsetHeight / 0.9)
    }
    var style = this.getTransformStyle(
      transform,
  ...
  kbd.style.left =
    (this.container.offsetWidth - kbd.offsetWidth / scale) / 2 + 'px'
  kbd.style.top =
    (this.container.offsetHeight - kbd.offsetHeight / scale) / 2 + 'px'
...
VT100.prototype.showContextMenu = function (x, y) {
  ...
  for (
    var node = menuentries.firstChild, i = 0;
    node;
    ...
    if (ch == undefined) {
      switch (key) {
        ...
        case 36:
          ...
          break
        case 37:
          /* Left         */ ch = this.cursorKeyMode ? '\u001BOD' : '\u001B[D'
          break
        case 38:
          /* Up           */ ch = this.cursorKeyMode ? '\u001BOA' : '\u001B[A'
          break
        case 39:
          /* Right        */ ch = this.cursorKeyMode ? '\u001BOC' : '\u001B[C'
          break
        case 40:
          /* Down         */ ch = this.cursorKeyMode ? '\u001BOB' : '\u001B[B'
          break
        case 45:
          /* Insert       */ ch = '\u001B[2~'
          break
  ...
  if (event.shiftKey || event.ctrlKey || event.altKey || event.metaKey) {
    ...
    if ((start = ch.substr(0, 2)) == '\u001B[') {
      for (
        ...
        (digit = ch.charCodeAt(part1.length)) >= 48 &&
        digit <= 57;
    ...
    if (inactive) {
      ...
      if (this.blinkingCursor) {
        this.cursor.className =
          this.cursor.className == 'bright' ? 'dim' : 'bright'
      } else {
...
VT100.prototype.getTransformStyle = function (transform, scale) {
  return scale && scale != 1.0
    ? transform == 'filter'
      ...
      : 'translateX(-50%) ' + 'scaleX(' + 1.0 / scale + ') ' + 'translateX(50%)'
    : ''
}
...
VT100.prototype.set80_132Mode = function (state) {
  ...
  if (transform) {
    ...
    var style = state ? this.getTransformStyle(transform, 1.65) : ''
      ...
      (function (win) {
        return function () {
          if (
            ...
            alert(
              'Attempted to print, but a popup blocker ' +
  ...
  try {
    switch (ch) {
      case 9:
        ...
        var print = doc.getElementById('print')
        var chars =
          print.lastChild && print.lastChild.nodeName == '#text'
            ? print.lastChild.textContent.length
            : 0
...
VT100.prototype.csiAt = function (number) {
  ...
  this.scrollRegion(
    this.cursorX,
    this.cursorY,
    this.terminalWidth - this.cursorX - number,
    1,
    number,
    0,
    this.color,
  ...
  switch (number) {
    case 0: // Erase from cursor to end of display
      this.clearRegion(
        this.cursorX,
        this.cursorY,
        this.terminalWidth - this.cursorX,
        1,
        this.color,
        this.style
      )
      if (this.cursorY < this.terminalHeight - 2) {
        this.clearRegion(
          ...
          this.cursorY + 1,
          this.terminalWidth,
          this.terminalHeight - this.cursorY - 1,
          this.color,
          this.style
        )
      }
...
VT100.prototype.csiL = function (number) {
  ...
  this.scrollRegion(
    ...
    this.cursorY,
    this.terminalWidth,
    this.bottom - this.cursorY - number,
    0,
    number,
    this.color,
    this.style
...
VT100.prototype.csim = function () {
  for (var i = 0; i <= this.npar; i++) {
    switch (this.par[i]) {
      ...
      case 1:
        this.attr =
          (this.attr & ~0x0400) /* ATTR_DIM */ | 0x0800 /* ATTR_BRIGHT */
        break
      case 2:
        this.attr =
      ...
      case 38:
        if (this.npar >= i + 2 && this.par[i + 1] == 5) {
          ...
          // not VT100 standard.
          this.attrFg =
            this.par[i + 2] >= 0 && this.par[i + 2] <= 255
              ? this.par[i + 2]
              : false
          i += 2
        } else {
          ...
          this.attr =
            (this.attr &
              ~(0x0400 /* ATTR_DIM */ | 0x0800 /* ATTR_BRIGHT */ | 0x0f)) |
      ...
      default:
        if (this.par[i] >= 30 && this.par[i] <= 37) {
          ...
          this.attr =
            ((this.attr & ~0xf0) | (bg << 4)) & ~(0x4000 /* ATTR_DEF_BG */)
          this.attrBg = false
        }
        break
    }
      ...
      switch (this.isEsc) {
        ...
        case 4 /* ESgotpars */:
          ...
          switch (ch) {
            ...
            /*a*/ case 0x61:
              this.gotoXY(
                this.cursorX + (this.par[0] ? this.par[0] : 1),
                this.cursorY
              )
              break
            /*D*/ case 0x44:
        ...
        case 19 /* ESss3 */:
          if (ch < 256) {
            ch =
              this.GMap[this.isEsc - 18 /* ESss2 */ + 2][
...
VT100.prototype.VT100GraphicsMap = [
  ...
  0x250c, 0x2514, 0x253c, 0xf800, 0xf801, 0x2500, 0xf803, 0xf804, 0x251c,
...
export function ShellInABox(url, container) {
  // var url = '/api-nac/cfg.php'
  // this.readurl = '/api-nac/shell.php'
      var url = '/cfg.php'
  this.readurl = '/shell.php'

  // var token = this.cookie('hash');
  // if (token == undefined) {
  //   token = ""
  // }
  // this.token = this.md5(token);
...
ShellInABox.prototype.sendRequest = function (request) {
  ...
  let url = this.session ? this.readurl : this.url
  ...
  var content =
    ...
    '&height=' +
    this.terminalHeight +
    '&controller=configMaintain&action=webshell' +
    (this.session
      ? '&session=' + encodeURIComponent(this.session)
      : '&rooturl=' + encodeURIComponent(this.rooturl)) +
    '&_token=' +
    this.token
...
ShellInABox.prototype.extendContextMenu = function (entries, actions) {
  ...
  for (
    var node = entries.firstChild, i = 0, j = 0;
    node;
    node = node.nextSibling
  ) {
    if (node.tagName == 'LI') {
      ...
      if (node.id == 'endconfig') {
        node.id = ''
        if (
          typeof serverSupportsSSL != 'undefined' &&
          serverSupportsSSL &&
...
export default function Trol() {
  useEffect(() => {
    // connect()
    window.shellinabox = new ShellInABox()
    setTimeout(() => {
      window.shellinabox.sendKeys('3F')
    }, 500)
  }, [])

  //连接
  // const connect = () => {
  //   post('/cfg.php?controller=configMaintain&action=set_webcmd', {
  //     webcmd_sw: 1,
  //   })
  // }
  ...
  return (
    <>
      <div style={{ display: 'block' }}>
        <div style={{ display: 'block', clear: 'both',margin:'12px' }}></div>
        <div
          id="vt100"
          sytle={{
            overflow: 'hidden',
